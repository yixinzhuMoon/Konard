	...
	if ($is_sticky.length > 0 ) {
		...
		$win.scroll(function(){
			...
			if ($win.width() > 991 || $is_sticky.hasClass('mobile-sticky')) {
				if($scroll > 100 ){
					if(!$is_sticky.hasClass('has-fixed')) {$is_sticky.addClass('has-fixed'); $('.logo-dark').hide(); $('.logo-light').show(); $('.navbar-nav').removeClass('header-nav');}
				} else {
					if($is_sticky.hasClass('has-fixed')) {$is_sticky.removeClass('has-fixed'); $('.logo-dark').show(); $('.logo-light').hide(); $('.navbar-nav').addClass('header-nav');}
				}
			} else {
				if($is_sticky.hasClass('has-fixed')) {$is_sticky.removeClass('has-fixed'); $('.logo-dark').show(); $('.logo-light').hide(); $('.navbar-nav').addClass('header-nav');}
			}
